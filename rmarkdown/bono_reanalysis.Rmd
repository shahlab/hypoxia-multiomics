...
d <- d %>% 
  column_to_rownames("samples") 
# Plot the heatmap
old.ortho.heatmap <- pheatmap::pheatmap(d, color = myColor, breaks = myBreaks, annotation_row = my_gene_row, annotation_colors = mycolors)
old.ortho.heatmap

ggsave(old.ortho.heatmap, filename = "../output/plots/plot.heatmap.human.hypoxia.orthologs.old.pdf", width = 35, height = 10)
...
```{r fig.width=35, fig.height=10}
ortho.new2 <- read_csv("../../new.orthologs", col_names = F)

d <- deseq.res %>%
  filter(gene_name %in% ortho.new2$X1) %>%
  select(samples, gene_name, log2FoldChange) %>%
  pivot_wider(names_from = gene_name, values_from = log2FoldChange) 
...
all.cor.fc <- pheatmap::pheatmap(cor.ortho)
all.cor.fc
...
only15.cor.fc <- pheatmap::pheatmap(cor.ortho)
only15.cor.fc

ggsave(all.cor.fc, filename = "../output/plots/plot.heatmap.cor.human.hypoxia.orthologs.all.genes.pdf", width = 30, height = 30)
ggsave(all.cor.fc, filename = "../output/plots/plot.heatmap.cor.human.hypoxia.orthologs.gt15data.genes.pdf", width = 30, height = 30)
